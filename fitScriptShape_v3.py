...
# if there si already one it does not delete it
    ...
    print("Created directory "+ path_dir)
else:
...
datacardfailName = "datacard_fail_" + var + ".txt"
os.system("cp "+ path_tools  + datacardfailName +" " + path_dir+ "/")
print("Copied datacard " + datacardfailName )
...
for line in fin:
    if 'REPLACE' in line:
        newline = line.replace('REPLACE_FILE_FAIL', '%s'%('datacard_fail_'+var +  ".root"))
        newline = newline.replace('REPLACE_FILE_PASS', '%s'%('datacard_pass_'+var +  ".root"))
